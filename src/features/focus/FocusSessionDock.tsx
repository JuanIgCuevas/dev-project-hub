...
export function FocusSessionDock() {
  ...
  if (session.status === 'completed') {
    return <aside className={`focus-session-dock completed focus-reflection ${positionClass}`}>
      ...
      <div className="focus-reflection-row"><label>¿Qué quedó pendiente?<input value={pending} onChange={event => setPending(event.target.value)} placeholder="Algo por revisar..." /></label><label>Mensaje para tu próxima sesión<input value={nextStep} onChange={event => setNextStep(event.target.value)} placeholder="Recordame continuar con..." /></label></div>
