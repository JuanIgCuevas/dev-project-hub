...
function IdeaForm({ idea, onClose }: { idea?: Idea; onClose: () => void }) {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      showToast(idea ? 'Idea actualizada correctamente.' : 'Idea creada correctamente.')
      onClose()
    } catch {
...
export function IdeasPage() {
  ...
  const changeStatus = async (idea: Idea, status: IdeaStatus) => {
    setActionError('')
    try { await updateIdea.mutateAsync({ id: idea.id, input: { title: idea.title, description: idea.description ?? '', technologies: idea.technologies, status, excitement: idea.excitement, usefulness: idea.usefulness, difficulty: idea.difficulty, portfolio_value: idea.portfolio_value, estimated_hours: idea.estimated_hours } }); showToast('Estado de la idea actualizado.') }
  ...
  const removeIdea = async (idea: Idea) => {
    ...
    setActionError('')
    try { await deleteIdea.mutateAsync(idea.id); showToast('Idea eliminada correctamente.') }
  ...
  const convertToProject = async (idea: Idea) => {
    ...
    try {
      ...
      showToast('Idea convertida en proyecto.')
