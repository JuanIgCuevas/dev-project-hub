...
export function PublicProjectPage() {
  ...
  return <div className="public-project-page">
    ...
    {project && <main className="public-project-content"><section className="public-project-hero"><div><div className="public-project-kicker"><span><Code2 /></span><p className="eyebrow">PROYECTO DESTACADO · POR <b data-no-translate>{project.owner_name}</b></p></div><h1 data-no-translate>{project.name}</h1><p className="public-project-description" data-no-translate={project.description ? true : undefined}>{project.description || 'Un proyecto construido con intención, foco y mejora continua.'}</p><div className="public-project-links">{project.live_url && <a className="button primary" href={project.live_url} target="_blank" rel="noreferrer"><Rocket /> Ver proyecto <ExternalLink /></a>}{project.repository_url && <a className="button" href={project.repository_url} target="_blank" rel="noreferrer"><Github /> Ver repositorio</a>}</div></div><aside><span className="public-status"><i />{statusLabels[project.status]}</span><div className="public-progress-ring" style={{ '--showcase-progress': `${progress * 3.6}deg` } as CSSProperties}><div><strong>{progress}%</strong><span>completado</span></div></div><p>{project.completed_tasks} de {project.total_tasks} tareas completadas</p></aside></section>
