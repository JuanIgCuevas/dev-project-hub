  ...
  return <section className={`momentum-section ${collapsed ? 'focus-planner-collapsed' : ''}`}>
    <article className={`focus-card ${collapsed ? 'collapsed' : ''}`}>
      ...
      {!collapsed && <>
        ...
        <div className="focus-task-picker">{candidateTasks.length ? candidateTasks.map((task, index) => { const selected = effectiveTaskIds.includes(task.id); return <button className={selected ? 'selected' : ''} type="button" onClick={() => toggleTask(task.id)} key={task.id}><span>{selected ? '✓' : index === 0 ? <Target /> : <CircleDot />}</span><div><small>{task.priority === 'high' ? 'PRIORIDAD ALTA' : task.priority === 'low' ? 'PRIORIDAD BAJA' : 'PRIORIDAD MEDIA'}</small><strong data-no-translate>{task.title}</strong>{task.due_date && <em><CalendarDays /> {new Intl.DateTimeFormat(preferences.language === 'en' ? 'en-US' : 'es-AR').format(new Date(`${task.due_date}T12:00:00`))}</em>}</div></button>}) : <p>Este proyecto no tiene tareas pendientes. Podés iniciar una sesión libre para definir el próximo paso.</p>}</div>
