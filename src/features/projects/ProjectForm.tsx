...
export function ProjectForm({ project }: { project?: Project }) {
  ...
  return <form className="project-form" onSubmit={handleSubmit(onSubmit)} noValidate>
    ...
    <div className="form-actions"><Link className="button" to={project ? `/projects/${project.id}` : '/projects'}>Cancelar</Link><button className="button primary" disabled={isSubmitting} type="submit">{isSubmitting ? 'Guardando...' : project ? 'Guardar cambios' : 'Crear proyecto'} {project ? <Save size={17} /> : <ArrowRight size={17} />}</button></div>
  </form>
}
