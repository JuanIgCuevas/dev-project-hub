...
import { NotFoundPage } from './components/AppFallbacks'
...
function MyTasksPage() {
  ...
  return <Shell><div className="tasks-page"><header className="tasks-heading"><div><p className="eyebrow">TU TRABAJO</p><h1>Mis tareas</h1><p>Todo lo pendiente, sin importar en qué proyecto esté.</p></div><div className="tasks-header-actions"><div className="view-toggle" role="group" aria-label="Cambiar vista"><button type="button" aria-pressed={viewMode === 'list'} className={viewMode === 'list' ? 'active' : ''} onClick={() => setViewMode('list')} title="Vista lista"><Rows3 size={17} /> Lista</button><button type="button" aria-pressed={viewMode === 'kanban'} className={viewMode === 'kanban' ? 'active' : ''} onClick={() => setViewMode('kanban')} title="Vista Kanban"><Columns3 size={17} /> Kanban</button></div><Link className="button primary" to="/projects"><FolderKanban size={17} /> Ver proyectos</Link></div></header>
